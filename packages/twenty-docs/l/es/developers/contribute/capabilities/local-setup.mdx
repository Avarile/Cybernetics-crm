...
<Tabs>
  <Tab title="Linux">
    **Opción 1 (preferida):** Para aprovisionar tu base de datos localmente:
...
Ahora puedes acceder a la base de datos en `localhost:5432`.

Si utilizaste la opción de Docker anterior, las credenciales predeterminadas son usuario `postgres` y contraseña `postgres`. Para instalaciones nativas de PostgreSQL, usa las credenciales y los roles configurados en tu máquina.
...
Puede iniciar sesión usando la cuenta demo por defecto: `avarile@gmail.com` (contraseña: `Avarile19840123!`)
