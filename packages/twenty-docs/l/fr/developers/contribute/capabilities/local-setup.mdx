...
<Tabs>
  ...
  <Tab title="Windows (WSL)">
    ...
    **Option 1 :** Pour provisionner PostgreSQL localement :
...
Vous pouvez maintenant accéder à la base de données à l'adresse `localhost:5432`.

Si vous avez utilisé l'option Docker ci-dessus, les identifiants par défaut sont l'utilisateur `postgres` et le mot de passe `postgres`. Pour les installations natives de PostgreSQL, utilisez les identifiants et les rôles configurés sur votre machine.
...
Vous pouvez vous connecter en utilisant le compte démo par défaut : `avarile@gmail.com` (mot de passe : `Avarile19840123!`)
